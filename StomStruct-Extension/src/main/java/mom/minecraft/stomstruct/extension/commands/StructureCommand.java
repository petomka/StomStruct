...
import mom.minecraft.stomstruct.core.structure.Rotation;
...
public class StructureCommand extends Command {
	public StructureCommand() {
		...
		ArgumentEnum<Rotation> rotationArgument = ArgumentType.Enum("rotation", Rotation.class);
		...
		setDefaultExecutor(((sender, context) -> {
			...
			sender.sendMessage("/" + context.getCommandName() + " rotate <rotation>: Rotate structure in your clipboard");
		...
		addSubcommand(new Command("rotate") {{ // double curly for instance initializer
			addSyntax((sender, context) -> {
				...
				if (StomStructExtension.playerLoadedStructures.containsKey(player.getUuid())) {
					Structure structure = StomStructExtension.playerLoadedStructures.get(player.getUuid());
					structure.rotate(context.get(rotationArgument));
				}
			}, rotationArgument);
		}});
