...
public class Structure {
	...
	public void rotate(Rotation rotation) {
		for (int i = 0; i < blockStatePalettes.length; i++) {
			for (int j = 0; j < blockStatePalettes[i].length; j++) {
				Block current = blockStatePalettes[i][j];
				Block rotated = BlockRotationHelper.rotateBlock(current, rotation);
				blockStatePalettes[i][j] = rotated;
			}
		}

		Function<short[], short[]> rotator = switch (rotation) {
			case NONE -> shorts -> shorts;
			case CLOCKWISE_90 -> shorts -> new short[]{(short) -shorts[2], shorts[1], shorts[0]};
			case CLOCKWISE_180 -> shorts -> new short[]{(short) -shorts[0], shorts[1], (short) -shorts[2]};
			case CLOCKWISE_270 -> shorts -> new short[]{shorts[2], shorts[1], (short) -shorts[0]};
		};

		for (int i = 0; i < blockPositions.length; i++) {
			blockPositions[i] = rotator.apply(blockPositions[i]);
		}
	}
	...
	public void buildWithPalette(@NotNull Point origin, @NotNull Instance instance, int paletteIndex) {
		...
		for (int i = 0; i < blockCount; i++) {
			...
			if (b.compare(Block.STRUCTURE_VOID, Block.Comparator.ID)) {continue;}
